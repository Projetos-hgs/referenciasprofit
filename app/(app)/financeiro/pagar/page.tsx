...
import { Search, ChevronDown, ChevronUp, ChevronsUpDown, ArrowUpCircle } from 'lucide-react'
...
export default function PagarPage() {
  ...
  const { data: kpis } = useSWR(
    selectedClient ? `/api/financeiro/kpis?clientId=${selectedClient.id}&type=payable` : null,
    fetcher
  )
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total em Aberto" value={formatCurrency(kpis?.open ?? 0)} variant="danger" icon={<ArrowUpCircle className="w-4 h-4" />} />
        <KpiCard title="Vencido" value={formatCurrency(kpis?.overdue ?? 0)} variant="warning" />
        <KpiCard title="Pago (mês)" value={formatCurrency(kpis?.paidMonth ?? 0)} variant="success" />
                ...
                <tr>
                  ...
                  <th className="text-left py-2.5 px-4 text-xs font-medium text-muted-foreground">
                    <button onClick={() => toggleSort('person_name')} className="flex items-center hover:text-foreground">
                      Fornecedor <SortIcon field="person_name" />
                    </button>
                  </th>
